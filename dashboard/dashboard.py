    ...
    if data_dir.exists():
        ...
        for category in ["recyclable", "compost", "landfill"]:
            ...
            if category_dir.exists():
                stats[category] = len(list(category_dir.glob("*.jpg"))) + len(list(category_dir.glob("*.png")))
        ...
        if stats["total"] > 0:
            ...
            df = pd.DataFrame({
                ...
                "Recyclable": [stats["recyclable"] // 4, (stats["recyclable"] // 2), (stats["recyclable"] // 4 * 3), stats["recyclable"]],
                "Compost": [stats["compost"] // 4, (stats["compost"] // 2), (stats["compost"] // 4 * 3), stats["compost"]],
                "Landfill": [stats["landfill"] // 4, (stats["landfill"] // 2), (stats["landfill"] // 4 * 3), stats["landfill"]],
            })
    else:
...
with col1:
    fig = px.line(df, x="Week", y=["Recyclable", "Compost", "Landfill"], markers=True,
...
with col2:
    ...
    if stats["total"] > 0:
        ...
        pie_data = pd.DataFrame({
            ...
            "Count": [stats["recyclable"] // 4, stats["compost"] // 4, stats["landfill"] // 4]
        })
    else:
        # Use sample data
        pie_data = pd.DataFrame({
...
st.info("After introducing the assistant, the school can reduce landfill waste by educating students with real-time feedback and visual progress tracking.")
